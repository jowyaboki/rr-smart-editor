    ...
    <div className="p-4 bg-[#131317] rounded-lg border border-[#2d2d35] space-y-3">
      ...
      <div className="space-y-2 text-xs">
        {Object.values(project.tracks).map(t => (
          <div key={t.id} className="flex justify-between items-center p-2 rounded bg-black bg-opacity-35">
            <span className="font-semibold text-gray-300">🔊 {t.name}</span>
            <span className="text-gray-400">➡️ Sends to: <span className="text-indigo-400 font-bold uppercase">{t.targetBusId}</span></span>
          </div>
        ))}
      </div>
    </div>
