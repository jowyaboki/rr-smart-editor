...
webHealth.registerCheck('Playback Engine', async () => ({
  status: 'healthy',
  responseTimeMs: 15,
  memoryBytes: 1024 * 1024 * 12,
}));
webHealth.registerCheck('Timeline Engine', async () => ({
  status: 'healthy',
  responseTimeMs: 5,
  memoryBytes: 1024 * 1024 * 4,
}));
webHealth.registerCheck('Render Engine', async () => ({
  status: 'healthy',
  responseTimeMs: 120,
  memoryBytes: 1024 * 1024 * 156,
}));
webHealth.registerCheck('Expression Engine', async () => ({
  status: 'healthy',
  responseTimeMs: 2,
  memoryBytes: 1024 * 1024 * 1,
}));
webHealth.registerCheck('Effects Engine', async () => ({
  status: 'healthy',
  responseTimeMs: 80,
  memoryBytes: 1024 * 1024 * 64,
}));
webHealth.registerCheck('AI Runtime', async () => ({
  status: 'healthy',
  responseTimeMs: 450,
