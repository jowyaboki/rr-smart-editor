...
export class NodeRegistry {
  ...
  public registerNode(node: Node): void {
    ...
    this.logEvent(
      'node_registered',
      'info',
      `Node registered successfully: ${node.name} (${node.id})`,
      { nodeId: node.id },
    );
  }
  ...
  public drainNode(id: string): boolean {
    ...
    this.logEvent('node_drained', 'warning', `Node entered draining state: ${node.name} (${id})`, {
      nodeId: id,
    });
    return true;
  }
    ...
    this.nodes.forEach((node) => {
      ...
      if (Date.now() - lastHbTime > timeoutMs && node.status !== 'offline') {
        ...
        this.logEvent(
          'heartbeat_lost',
          'error',
          `Heartbeat timeout. Node offline: ${node.name} (${node.id})`,
          { nodeId: node.id },
        );
      }
    });
