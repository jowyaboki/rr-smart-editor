...
describe('Digital Twin Simulation Engine Core Unit Tests', () => {
  ...
  test('Isolated Digital Twin - State isolation and immutability', async () => {
    ...
    const simulatedIntro = twin
      .getProjectState()
      .timeline.clips.find((c: any) => c.id === 'clip_1');
  ...
  test('Rollback & Time Travel', async () => {
    ...
    assert.strictEqual(
      twin.getVariables().find((v) => v.name === 'bitrate'),
      undefined,
    ); // Reverted!
  ...
  test('Parallel Simulations - Fully independent execution', async () => {
    ...
    await twinA.executeOperation('timeline_edit', {
      action: 'move',
      clipId: 'clip_1',
      startFrame: 100,
    });
    await twinB.executeOperation('timeline_edit', {
      action: 'move',
      clipId: 'clip_1',
      startFrame: 200,
    });
  ...
  test('Predictive Simulation & Accuracy', async () => {
    ...
    assert.ok(
      largeResult.predictions.performance.renderDurationMs >
        result.predictions.performance.renderDurationMs,
    );
  });
        ...
        clips: [
          {
            ...
            duration: 100,
            type: 'video',
            expression: 'this.__proto__.pollute()',
          },
        ],
      },
    };
  ...
  test('Plugin Extensibility - Custom adapters, prediction providers, validators, and strategies', async () => {
      ...
      validate: async (twin) => [
        {
          ...
          severity: 'error',
          message: 'Custom conflict detected!',
        },
      ],
    };
    ...
    const customStrategy: OptimizationStrategy = {
      ...
      analyze: async (twin) => [
        {
          id: 'custom_prop',
          category: 'proxy_recommendations',
          title: 'Custom Proxy Rec',
          description: 'Desc',
          potentialSavings: {
            costReduction: 10,
            renderDurationReductionMs: 5000,
    ...
    assert.ok(
      result.validationIssues.some((issue) => issue.message === 'Custom conflict detected!'),
    );
